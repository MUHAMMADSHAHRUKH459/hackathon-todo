          ...
          {tasks.length === 0 ? (
            ...
            <div className="space-y-4">
              {tasks.map((task, index) => (
                <motion.div key={task.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} transition={{ delay: index * 0.05 }} whileHover={{ scale: 1.02, x: 10 }} className={`rounded-2xl p-6 shadow-lg border-l-4 transition-all ${task.completed ? darkMode ? 'border-green-500 bg-gray-800/70' : 'border-green-500 bg-green-50/50' : darkMode ? 'border-blue-500 bg-gray-800/50' : 'border-blue-500 bg-white'}`}>
